...
# Production base URL — set SITE_URL env var on Render, falls back for local dev
BASE_URL = os.environ.get("SITE_URL", "https://joya-site-1.onrender.com")

# ALL pages are public (no login required)
PUBLIC_PATHS = {"/index.html", "/admin.html", "/favicon.ico", "/robots.txt", "/sitemap.xml", "/google758877207bc20678.html", "/health"}
...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            return self._send_json({"status": "ok", "version": os.environ.get("APP_VERSION", "1.0.0")})
    ...
    def _serve_download(self):
        ...
        if os.path.isfile(exe_path):
            ...
            try:
                ...
                # Inject production BASE_URL into the installer script
  ...
  "explanation": "Brief 1-2 sentence human-like constructive explanation."
        ...
        if path == "/api/admin/push_update":
            ...
            with open(update_flag_path, "w", encoding="utf-8") as f:
                json.dump({"version": version, "url": f"{BASE_URL}/download", "pushed_at": now_iso()}, f)
            with db() as conn:
        ...
        if path == "/api/client-sync":
            ...
            with db() as conn:
                ...
                if update_info.get("version"):
                    ...
                    resp["update_url"] = update_info.get("url", f"{BASE_URL}/download")
...
def main():
    ...
    print(f"  Port     : {PORT}")
    print(f"  Local    : http://{HOST if HOST != '0.0.0.0' else '127.0.0.1'}:{PORT}/")
